...
function nstGetSubtree ($thandle, $node, $attributes)
/*  */
{
  $tree = array();	
  ...
  while ($curr = nstWalkNext($thandle, $wlk)) {
	// print indentation
	//print (str_repeat("&nbsp;", nstWalkLevel($thandle, $wlk)*4));
	$tree[] = array( 'level' =>  nstWalkLevel($thandle, $wlk),);
	// print attributes
...
function nstGetTree ($thandle, $attributes = '')
/* Prints attributes of the entire tree. */
{ 
  nstGetSubtree ($thandle, nstRoot($thandle), $attributes);
}




function nstBreadcrumbsString ($thandle, $node,$attribute)
...
{
  // current node
  $ret = nstNodeAttribute ($thandle, $node, $attribute);
  // treat ancestor nodes
  while(nstAncestor ($thandle, $node) != array("l"=>0,"r"=>0)){
    $ret = "".nstNodeAttribute($thandle, nstAncestor($thandle, $node), $attribute)." &gt; ".$ret;
